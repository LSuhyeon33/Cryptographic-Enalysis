"""
=========================================
TC20R - Toy Cipher (encryption/decryption)
 - version 3:
            R: Reduced Round (부분 라운드 암호화 기능을 추가)
            Whitening(화이트닝) 추가 - 암호문 생성 직전에 라운드키 XOR 추가
 - version 2: 
      동일한 라운드함수만 사용함
      키 스케줄 없음
      복호화 추가
=========================================
"""

NUM_ROUND = 10

Sbox = [
        
       ]

ISbox = [
        
    ]


#--------
# AR: Add Roundkey
def AR(in_state, rkey):
    out_state = [0] * len(in_state)
    for i in range(len(in_state)):
        out_state[i] = in_state[i] ^ rkey[i] 
    return out_state

#--------
# SB: Sbox layer
def SB(in_state):
    out_state = [0] * len(in_state)
    for i in range(len(in_state)):
        out_state[i] = Sbox[in_state[i]]
    return out_state

#--------
# LM: Linear Map
def LM(in_state):
    out_state = [0] * len(in_state)
    All_Xor = in_state[0] ^ in_state[1] ^ in_state[2] ^ in_state[3]
    for i in range(len(in_state)):
        out_state[i] = All_Xor ^ in_state[i]
    return out_state

#--------
# Enc_Round
def Enc_Round(in_state, rkey):
    out_state = [0] * len(in_state)
    out_state = AR(in_state, rkey)
    in_state = SB(out_state)
    out_state = LM(in_state)
    return out_state

#--------
# TC20R Encryption (전체 라운드 암호화)
def TC20R_Enc(PT, key):
    NROUND = NUM_ROUND # 라운드 수 = 10
    CT = PT #CT = [0] * len(PT)
    for i in range(NROUND):
        CT = Enc_Round(CT, key)
    #암호문 생성 직전 화이트닝 추가
    CT = AR(CT, key)
    return CT

# TC20R Encryption (부분 라운드 암호화)
def TC20R_Enc_Reduced_Round(PT, key, numRound):
    CT = PT #CT = [0] * len(PT)
    for i in range(numRound):  # 주어진 라운드 수 만큼만 암호화
        CT = Enc_Round(CT, key)
    #암호문 생성 직전 화이트닝 추가
    CT = AR(CT, key)
    return CT

#--------------------------------------
#  Decryption
#--------------------------------------

#-- SB: Sbox layer
def ISB(in_state):
    out_state = [0, 0, 0, 0]
    for i in range(len(in_state)):
        out_state[i] = ISbox[in_state[i]]
    return out_state

#-- Decrypt Round
def Dec_Round(in_state, rkey):
    out_state1 = [0, 0, 0, 0]
    out_state2 = [0, 0, 0, 0]
    out_state3 = [0, 0, 0, 0]
    out_state1 = LM(in_state)
    out_state2 = ISB(out_state1)
    out_state3 = AR(out_state2, rkey)
        
    return out_state3

#-- TC20R Decryption (부분 라운드 복호화)
def TC20R_Dec(input_state, key):
    # 복호화 시작전 화이트닝 추가
    state = AR(input_state, key)
    numRound = NUM_ROUND # 라운드 수
    for i in range(0, numRound):
        state = Dec_Round(state, key)

    return state

#-- TC20R Decryption (전체 라운드 복호화)
def TC20R_Dec_Reduced_Round(input_state, key, numRound):
    # 복호화 시작전 화이트닝 추가
    state = AR(input_state, key)
    for i in range(0, numRound):
        state = Dec_Round(state, key)

    return state
#-----------
# main()
def main():
    message = 'ARIA'
    key = [0, 1, 2, 3]
    #PT = list(message)
    PT = [ ord(ch) for ch in message ]
    print('Message =', message)
    print('PT =', PT)
    #CT = TC20R_Enc(PT, key)
    CT = TC20R_Enc_Reduced_Round(PT, key, 5)
    #print('PT =', PT)
    print('CT =', CT)
    hexPT = [hex(item) for item in PT]
    hexCT = [hex(item) for item in CT]
    print('hexPT =', hexPT)
    print('hexCT =', hexCT)
    bytePT = bytes(PT)
    print('bytePT =', bytePT)
    byteCT = bytes(CT)
    print('byteCT =', byteCT)
    
    #input_state = [202, 134, 119, 230]  # for TC20
    #input_state = [202, 135, 117, 229]  # for TC20R
    input_state = [219, 45, 158, 55]    # for TC20R 5라운드 암호화
    #output_state = [ item for item in TC20_Dec(input_state, key)  ]
    #output_state = TC20R_Dec(input_state, key)    
    output_state = TC20R_Dec_Reduced_Round(input_state, key, 5)
    print('input ciphertext =', input_state)
    print('output plaintext =', output_state)
        
    
#----- Run main()
if __name__ == '__main__':
    main()
    
'''
- version 3 TC20R - 5라운드 암호화
Message = ARIA
PT = [65, 82, 73, 65]
CT = [219, 45, 158, 55]
hexPT = ['0x41', '0x52', '0x49', '0x41']
hexCT = ['0xdb', '0x2d', '0x9e', '0x37']
bytePT = b'ARIA'
byteCT = b'\xdb-\x9e7'
input ciphertext = [219, 45, 158, 55]
output plaintext = [65, 82, 73, 65]

- version 3 TC20R
Message = ARIA
PT = [65, 82, 73, 65]
CT = [202, 135, 117, 229]
hexPT = ['0x41', '0x52', '0x49', '0x41']
hexCT = ['0xca', '0x87', '0x75', '0xe5']
bytePT = b'ARIA'
byteCT = b'\xca\x87u\xe5'
input ciphertext = [202, 135, 117, 229]
output plaintext = [65, 82, 73, 65]
    
- version 2 TC20
Message = ARIA
PT = [65, 82, 73, 65]
CT = [202, 134, 119, 230]
hexPT = ['0x41', '0x52', '0x49', '0x41']
hexCT = ['0xca', '0x86', '0x77', '0xe6']
bytePT = b'ARIA'
byteCT = b'\xca\x86w\xe6'
input ciphertext = [202, 134, 119, 230]
output plaintext = [65, 82, 73, 65]

''' 



